...
//
// ===================================================================================
// Port to CH552E by DazzlingOkami
...
void UART_ISR(void) __interrupt(INT_NO_UART1) __using(1) {
  UART_interrupt();
}
...
void main(void) {
  ...
  while(1) {
    ...
    if(UART_available() && CDC_ready()) {
      while(UART_available() && CDC_ready()) CDC_write(UART_read());
      CDC_flush();
    }
  }
}
